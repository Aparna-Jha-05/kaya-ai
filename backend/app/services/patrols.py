...
class ConstraintGraph:
    ...
    cooling_plant_max_kw: float | None = None
    # Water system limits
...
class PatrolEngineService:
    ...
    def run_all_patrols(cls, bid: VendorBidExtract, graph: ConstraintGraph | None = None) -> DocketScorecard:
        ...
        if (
            graph.cooling_plant_max_kw is not None
            and equipment.cooling_capacity_kw is not None
            and exceeds(equipment.cooling_capacity_kw, graph.cooling_plant_max_kw)
        ):
        ...
        results.append(PatrolResult(patrol_name="TRAFFIC_CONTROL", status=traffic_status,
            ...
            evidence={
                "promised_delivery_weeks": bid.promised_delivery_weeks,
                "maximum_delivery_weeks": graph.maximum_delivery_weeks,
