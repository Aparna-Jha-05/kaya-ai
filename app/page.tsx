    ...
    <div className="space-y-6 pb-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-text">Procurement review</h1>
          <p className="mt-1 max-w-2xl text-sm text-text/50">
            IIT Smart Campus Phase 1 · chilled-water plant. AI extracts bid data; deterministic checks provide the evidence.
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs text-text/40">
          <span className="inline-flex items-center gap-1.5">
            <span className="h-1.5 w-1.5 rounded-full bg-blue" /> Demo data
          </span>
          <MockBadge />
        </div>
      </div>
      ...
      <section aria-labelledby="investigation-heading" className="space-y-4 rounded-xl border border-white/10 bg-card/40 p-4 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 id="investigation-heading" className="mt-1 text-lg font-semibold text-text">Evidence for a reviewer decision</h2>
            <p className="mt-1 text-sm text-text/50">Review checks and commercial exposure. This workspace does not approve or reject purchase orders.</p>
          </div>
          ...
          <div className="space-y-6 lg:col-span-5">
            ...
            {requiresRfi ? (
              <div className="rounded-xl border border-violet/30 bg-violet/5 p-4">
                <p className="font-mono text-[11px] uppercase tracking-widest text-violet">Review required</p>
                <h3 className="mt-1 text-sm font-semibold text-text">Request a compliant specification</h3>
                <p className="mt-1 text-xs leading-relaxed text-text/60">The draft cites the recorded failures. Review and approve it before sending it to the workflow.</p>
