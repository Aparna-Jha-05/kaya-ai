...
function EvidenceTab({ bid }: { bid: Bid }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <CardHeader title="Document forensics" caption="Metadata supports reviewer context; it is not proof of coordination." />
        <div className="flex flex-wrap gap-2 p-4 text-xs">
          <span className="rounded border border-white/10 bg-surface px-2 py-1 font-mono text-text/55">PDF author: proposal author</span>
          <span className="rounded border border-white/10 bg-surface px-2 py-1 font-mono text-text/55">Creation tool: submitted PDF</span>
...
function PatrolEnhancements({ bid, patrol }: { bid: Bid; patrol: "building" | "green" | "vice" | "traffic" }) {
  ...
  const content = patrol === "building"
    ? ["Constraint source: Live MCP Sync · demo-Revit snapshot", "Substitution envelope: recorded constraints remain the same for the proposed equipment"]
    : patrol === "green"
      ? [`Market benchmark: ${market.label.replace(" · anomaly", "")} ${market.anomaly ? "— review specification" : "— within normal range"}`]
      : patrol === "vice"
        ? [`Agreement Compliance Index: ${integrity.aci}/100`, `Bid Integrity Matrix: ${integrity.summary}`, `Forward reliability forecast: ${(integrity.aci > 0 || (bid.vendor_history.late_deliveries / bid.vendor_history.total_deliveries) >= 0.4) ? "risk escalating" : "stable"}`, "Dialogue verification: no active transcripts"]
        : ["Float-aware safe order date: 02 Aug 2026", "Dynamic re-validation: rerun checks when post-award specifications change"];
  return <div className="mt-3 border-t border-white/5 pt-3"><p className="font-mono text-[9px] uppercase tracking-wider text-text/40">Enriched constraint graph</p><ul className="mt-1.5 space-y-1 text-[11px] text-text/55">{content.map((item) => <li key={item}>• {item}</li>)}</ul></div>;
}
...
function ActivityTab({ bid }: { bid: Bid }) {
  ...
  const dialogueItem = bid.id === "B" ? <li className="px-4 py-3"><p className="text-sm text-text/80">Dialogue verification <span className="text-text/40">· no active transcript</span></p><p className="mt-1 text-xs text-text/50">When a verified transcript is available, compare stated payment terms with clause 3.4 before recording a workflow action.</p></li> : null;
  return <Card><CardHeader title="Bid activity" caption="Decision and workflow events for this bid. The full cross-project record is available in Audit trail." />{matched.length || dialogueItem ? <ul className="divide-y divide-white/5">{dialogueItem}{matched.map((row, index) => <li key={`${row.timestamp}-${index}`} className="px-4 py-3"><p className="text-sm text-text/80">{row.action} <span className="text-text/40">· {row.patrol}</span></p><p className="mt-1 text-xs text-text/50">{row.evidence}</p></li>)}</ul> : <p className="p-5 text-sm text-text/55">No bid-specific events have been recorded in this browser session.</p>}</Card>;
}
