    ...
    <div className="space-y-5">
      ...
      {comparisonBids.length > 0 ? (
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_310px]">
          <Card>
            <CardHeader title="Bid comparison" caption={`Showing ${comparisonBids.length} selected bid${comparisonBids.length === 1 ? "" : "s"}. Select a row for details.`} right={<span className="inline-flex items-center gap-1.5 text-xs text-text/50"><SlidersHorizontal className="h-3.5 w-3.5 text-blue" /> {filterLabel(complianceFilter)}</span>} />
