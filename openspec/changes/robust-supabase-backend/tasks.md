...
- [x] 0.1 Review `/api/v1` request, response, error, and lifecycle scenarios with the frontend owner
- [x] 0.2 Obtain domain/QA approval for door-clearance semantics, carbon functional unit, Vice Squad scoring, and TCO² units
...
- [x] 5.2 Update Green Patrol to require an approved, matching carbon functional unit
- [ ] 5.3 Implement Vice Squad `pgvector` search using a pinned embedding model and cited historical records
- [x] 5.4 Implement the approved, bounded Vice Squad risk formula and missing-history flag
- [x] 5.5 Implement Traffic Control with explicit currency, time horizon, penalty, carbon basis, and intermediate terms
...
- [x] 7.2 Implement officer decision updates (`OfficerDecision`) with `expected_version` concurrency
- [x] 7.4 Preserve officer decisions when automated assessments change

## 8. Query APIs and Shared Contract

- [x] 8.1 Implement array-returning `GET /api/v1/bids` and detailed `GET /api/v1/bids/{id}` (keeping frontend array contract stable)
- [ ] 8.2 Implement project-scoped `GET /api/v1/suppliers` with verified coordinate provenance
- [x] 8.3 Implement `GET /api/v1/audit/logs` using ActivityEvent evidence details
- [x] 8.4 Implement bounded, non-persisted `POST /api/v1/bids/simulate`
- [x] 8.5 Add consistent error responses, 409 conflict handling on stale versions, and readiness reporting

## 9. Frontend Integration

- [x] 9.1 Add typed API helpers in `lib/api.ts`
- [x] 9.2 Connect work queue, portfolio, and review screens
- [x] 9.3 Connect evidence, dimension, RFI, supplier, and audit screens
